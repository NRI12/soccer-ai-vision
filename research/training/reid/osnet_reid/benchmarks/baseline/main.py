...
def build_datamanager(cfg):
    if cfg.data.type == 'image':
        return osnet_reid.data.ImageDataManager(**imagedata_kwargs(cfg))
    else:
        return osnet_reid.data.VideoDataManager(**videodata_kwargs(cfg))
...
def build_engine(cfg, datamanager, model, optimizer, scheduler):
    ...
    if cfg.data.type == 'image':
        ...
        if cfg.loss.name == 'softmax':
            engine = osnet_reid.engine.VideoSoftmaxEngine(
                datamanager,
                model,
                optimizer=optimizer,
                scheduler=scheduler,
            ...
            engine = osnet_reid.engine.VideoTripletEngine(
                datamanager,
                model,
                optimizer=optimizer,
                margin=cfg.loss.triplet.margin,
...
def main():
    ...
    if cfg.use_gpu:
        ...
        torch.backends.cudnn.deterministic = not cfg.train.cudnn_benchmark
    ...
    optimizer = osnet_reid.optim.build_optimizer(model, **optimizer_kwargs(cfg))
